...
dnf5_query_thread(PkBackendJob *job, GVariant *params, gpointer user_data)
{
	...
	try {
		if (role == PK_ROLE_ENUM_SEARCH_NAME || role == PK_ROLE_ENUM_SEARCH_DETAILS
		    || role == PK_ROLE_ENUM_SEARCH_FILE || role == PK_ROLE_ENUM_RESOLVE
			...
			for (int i = 0; values[i]; i++)
				search_terms.push_back(values[i]);
			...
			if (role == PK_ROLE_ENUM_GET_UPDATES) {
				...
				for (const auto &adv_pkg : adv_query.get_advisory_packages_sorted(pkg_set)) {
					std::string key = adv_pkg.get_name() + ";" + adv_pkg.get_evr() + ";"
							  + adv_pkg.get_arch();
			...
			role == PK_ROLE_ENUM_GET_DETAILS || role == PK_ROLE_ENUM_GET_FILES
			|| role == PK_ROLE_ENUM_DOWNLOAD_PACKAGES || role == PK_ROLE_ENUM_GET_UPDATE_DETAIL) {
			g_auto(GStrv) package_ids = NULL;
			if (role == PK_ROLE_ENUM_DOWNLOAD_PACKAGES) {
				...
				for (auto &pkg : pkgs) {
					...
					if (!path.empty())
						files_c.push_back(g_strdup(path.c_str()));
				}
				...
				for (auto &pkg : pkgs) {
					...
					if (it != pkg_to_adv_pkg.end()) {
						...
						for (const auto &ref : advisory.get_references()) {
							if (ref.get_url().empty())
								continue;
							if (ref.get_type() == "bugzilla")
								bugzilla_urls.push_back(ref.get_url());
							else if (ref.get_type() == "cve")
								cve_urls.push_back(ref.get_url());
							else if (ref.get_type() == "vendor")
								vendor_urls.push_back(ref.get_url());
						}
						...
						if (it->second.get_reboot_suggested())
							restart = PK_RESTART_ENUM_SYSTEM;
						else if (it->second.get_restart_suggested())
							restart = PK_RESTART_ENUM_APPLICATION;
						else if (it->second.get_relogin_suggested())
							restart = PK_RESTART_ENUM_SESSION;

						g_auto(GStrv) bugzilla_strv = (gchar **) g_new0(
							gchar *,
							bugzilla_urls.size() + 1);
						for (size_t i = 0; i < bugzilla_urls.size(); i++)
							bugzilla_strv[i] = g_strdup(bugzilla_urls[i].c_str());
						g_auto(GStrv)
							cve_strv = (gchar **) g_new0(gchar *, cve_urls.size() + 1);
						for (size_t i = 0; i < cve_urls.size(); i++)
							cve_strv[i] = g_strdup(cve_urls[i].c_str());
						g_auto(GStrv) vendor_strv = (gchar **) g_new0(
							gchar *,
							vendor_urls.size() + 1);
			...
			for (auto &pkg : pkgs) {
				...
				if (role == PK_ROLE_ENUM_GET_DETAILS) {
					...
					for (const auto &f : files_vec)
						files_c.push_back(const_cast<char *>(f.c_str()));
					files_c.push_back(nullptr);
			...
			for (auto repo : query) {
				...
				if (id == "@System" || id == "@commandline")
					continue;
				if (!dnf5_backend_pk_repo_filter(*repo, filters))
					continue;
...
dnf5_transaction_thread(PkBackendJob *job, GVariant *params, gpointer user_data)
{
	...
	try {
			...
			if (std::filesystem::exists(rpm_db_path) && std::filesystem::is_directory(rpm_db_path)) {
				for (const auto &entry : std::filesystem::directory_iterator(rpm_db_path)) {
					if (entry.is_regular_file()
					    && entry.path().filename().string().starts_with("__db.")) {
		...
		if (res != libdnf5::base::Transaction::TransactionRunResult::SUCCESS) {
			std::string msg;
			for (const auto &p : trans.get_transaction_problems())
				msg += p + "; ";
			pk_backend_job_error_code(
				job,
		...
		if (role == PK_ROLE_ENUM_REPO_ENABLE || role == PK_ROLE_ENUM_REPO_SET_DATA) {
			...
			if (!trans.get_transaction_packages().empty()) {
				for (const auto &item : trans.get_transaction_packages()) {
					...
					PkInfoEnum info = PK_INFO_ENUM_UNKNOWN;
					if (action == libdnf5::transaction::TransactionItemAction::INSTALL
					    || action == libdnf5::transaction::TransactionItemAction::UPGRADE)
						info = PK_INFO_ENUM_INSTALLING;
					else if (
						action == libdnf5::transaction::TransactionItemAction::REMOVE
						|| action == libdnf5::transaction::TransactionItemAction::REPLACED)
						info = PK_INFO_ENUM_REMOVING;
					else if (action == libdnf5::transaction::TransactionItemAction::REINSTALL)
						info = PK_INFO_ENUM_REINSTALLING;
					else if (action == libdnf5::transaction::TransactionItemAction::DOWNGRADE)
